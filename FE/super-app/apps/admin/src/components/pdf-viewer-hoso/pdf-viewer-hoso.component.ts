...
@Component({
    selector: 'app-pdf-viewer-hoso',
    standalone: true,
    imports: [CommonModule, PdfViewerModule],
    templateUrl: './pdf-viewer-hoso.component.html',
    styleUrls: ['./pdf-viewer-hoso.component.scss']
})
export class PdfViewerHoSoComponent implements OnInit, OnDestroy {
    @Input() document?: any;
    ...
    constructor(
        private documentService: PortfolioService
    ) { }
    ...
    private loadFile() {
        ...
        this.documentService.viewFile(this.document.id)
    ...
    onPdfLoadError(error: any) {
        ...
        this.loading = false;
        this.error = 'Không thể tải file PDF. File có thể bị hỏng hoặc không đúng định dạng PDF.';
    }
    ...
    downloadFile() {
        ...
        this.documentService.downloadFile(this.document.id)
            .pipe(takeUntil(this.destroy$))
            .subscribe({
                next: (blob: Blob) => {
